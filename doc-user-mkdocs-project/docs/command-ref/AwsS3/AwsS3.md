...
**<p style="text-align: center;">
![AwsS3-list](AwsS3-list.png)
</p>**

**<p style="text-align: center;">
`AwsS3` Command Editor for List Parameters (<a href="../AwsS3-list.png">see also the full-size image)</a>
</p>**
...
|***List***|`MaxKeys`| Maximum number of keys to list per request.  AWS limits the number of objects returned per request to 1000 and `MaxKeys` must be <= 1000. | `1000` (AWS limit). |
...
||`UploadFiles`| List of files to upload using syntax: `file1:key1,file2:key2`, where `file` is a local file name and the `key` identifies an S3 object, can use `${Property}` syntax. | |
