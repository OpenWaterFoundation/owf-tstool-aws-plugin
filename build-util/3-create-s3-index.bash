#!/bin/bash
#
# Create the TSTool AWS plugin product 'index.html' page on Amazon S3,
...
#
# software.openwaterfoundation.org/tstool-aws-plugin/index.html
# software.openwaterfoundation.org/tstool-aws-plugin/index.csv
#
...
#   2018-12-04 16:16:22          0 tstool-aws-plugin/
#   2018-12-04 16:16:37          0 tstool-aws-plugin/1.0.0/
#   2018-12-04 16:17:19   46281975 tstool-aws-plugin/1.0.0/tstool-aws-plugin-win-1.0.0-2201020304.zip
createCatalogFile() {
  ...
  if [ ${errorCode} -ne 0 ]; then
    logError ""
    logError "Error listing TSTool AWS plugin download files to create catalog (maybe missing --aws-profile)?"
    exit 1
  fi
  ...
  tmpS3CatalogPath="/tmp/${USER}-tstool-aws-plugin-catalog-ls-installers.txt"
  # Only list installers that match the current download filename specification
  cat ${tmpS3ListingPath} | grep -E 'tstool-aws-plugin.*.zip' > ${tmpS3CatalogPath}
  logInfo "Available TSTool AWS plugin installers:"
...
createIndexHtmlFiles() {
  ...
  indexHtmlTmpFile="/tmp/${USER}-tstool-aws-plugin-index.html"
...
<li>The TSTool AWS Plugin requires that TSTool is also installed if not already installed:
    <ul>
...
<li> Follow the latest <a href="https://software.openwaterfoundation.org/tstool-aws-plugin/latest/doc-user/appendix-install/install/">installation instructions</a>
    ...
    cat "${tmpS3CatalogPath}" | grep "${downloadOs}-" | awk '{ printf "%s %s %s %s\n", $1, $2, $3, $4 }' | sed -E 's|([0-9][0-9]/)|\1-zzz|g' | sed 's|/-zzz|-zzz|g' | sed 's|dev|-dev|g' | sort -r -k4,4 | sed 's|-zzz||g' | sed 's|-dev|dev|g'  | awk '
      {
        ...
        downloadFileProduct=""            # The product name (e.g., TSTool AWS Plugin)
        ...
        if ( filenameSpecVersion = 1 ) {
          # Initial filename spec using format:  tstool-aws-plugin/1.0.0.dev/software/tstool-aws-plugin-1.0.0.dev-win-2201020304.zip
          ...
          downloadFileProduct="tstool-aws-plugin"
...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Create the software.openwaterfoundation.org/tstool-aws-plugin/index.html and related files."
...
uploadIndexFiles() {
  ...
  logInfo "Assuming that TSTool AWS Plugin installer files were previously uploaded to S3."
  ...
  if [ -f "${s3CatalogTxtFileUrl}" ]; then
    ...
    if [ ${errorCode} -ne 0 ]; then
      logError ""
      logError "Error uploading TSTool AWS Plugin catalog file."
      # Not fatal.
    fi
  else
  ...
  if [ "${operatingSystem}" = "mingw" ]; then
    ...
    MSYS_NO_PATHCONV=1 ${awsExe} cloudfront create-invalidation --distribution-id "${cloudFrontDistributionId}" --paths '/tstool-aws-plugin/index.html' '/tstool-aws-plugin' '/tstool-aws-plugin/' "/tstool-aws-plugin/${indexFaviconFile}" --output json --profile "${awsProfile}" | tee ${tmpFile}
  else
    ${awsExe} cloudfront create-invalidation --distribution-id "${cloudFrontDistributionId}" --paths '/tstool-aws-plugin/index*' '/tstool-aws-plugin' '/tstool-aws-plugin/' "/tstool-aws-plugin/${indexFaviconFile}" --output json --profile "${awsProfile}" | tee ${tmpFile}
  fi
...
user=""
tmpS3ListingPath="/tmp/${USER}-tstool-aws-plugin-catalog-ls.txt"
